function paper_equations_run(step_folder)
%% =========================================================
% Display header
%% =========================================================
disp('****************************************************************************')
disp(' This code take as input the Surface Gradient [%] and gives prediction')
disp(' of lower limb joint kinematics and kinetics of one gait cycle during running at 2.25m/s')
disp('****************************************************************************')
...
%%%%%%%%%
Surface_Gradient = getUserChoice('Enter Surface Gradient [%] input range is -10% to 10%: ', -10:10);
yesF = getUserChoice('Type 1 to plot data, 2 for no plot: ', [1 2]);

anat_plane = 'Sagittal';
CSV_path = ['Coefs\' anat_plane  '\RUN\'];

load model_coefs
s_size_all = eval(['model_coefs.' anat_plane '.s_size_run']);
count=1;
...
save(fullfile(step_folder, ['ALLDATAfinal' sg_str '.mat']), 'ALLDATA');
...
if yesF==1
    ...
    title(sprintf('%s   ,Running at 2.25m/s, %s %1.f %s', joint1{1}, 'Surface Gradient =',Surface_Gradient,'%'))
    ylabel('Angle [Degree]');
    ...
    title(sprintf('%s   ,Running at 2.25m/s, %s %1.f %s', joint1{2}, 'Surface Gradient =',Surface_Gradient,'%'))
    ylabel('Angle [Degree]');
    ...
    title(sprintf('%s   ,Running at 2.25m/s, %s %1.f %s', joint1{3}, 'Surface Gradient =',Surface_Gradient,'%'))
    ylabel('Angle [Degree]');
